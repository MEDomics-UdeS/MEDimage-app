...
const FlowCanvas = ({ workflowType, setWorkflowType }) => {
  ...
  useEffect(() => {
    ...
    if (groupNodeId.id != "MAIN") {
      ...
      // Else the workflow is an learningMEDiml workflow
      setWorkflowType("learningMEDiml")
      // Hide the nodes that are not in the learningMEDiml group
      hideNodesbut(groupNodeId.id)
    }
  }, [groupNodeId])
      ...
      flow.nodes.forEach((node) => {
        ...
        if (node.type === "featuresNode") {
          ...
          modifiedFlow.drawflow[moduleName].data[nodeID] = {
            id: nodeID,
            name: node.data.setupParam.possibleSettings.defaultSettings.MEDimlName,
      ...
      const restoreFlow = async () => {
        try {
          ...
          Object.values(flow.nodes).forEach((node) => {
            ...
            // set workflow type
            let subworkflowType = node.data.internal.subflowId != "MAIN" ? "learningMEDiml" : "learningMEDiml"
            // set node type
  ...
  const onLoadDeafult = useCallback(() => {
    ...
    if (confirmation) {
      ...
      const restoreFlow = async () => {
        try {
          const flow = require("../../public/setupVariables/possibleSettings/learningMEDiml/learningMEDimlDefaultWorkflow.json")
          console.log("loaded flow", flow)
      ...
      <WorkflowBase
        ...
        ui={
          <>
            ...
            <div className="panel-bottom-center">{isProgressUpdating && <ProgressBarRequests progressBarProps={{ animated: true, variant: "success" }} isUpdating={isProgressUpdating} setIsUpdating={setIsProgressUpdating} progress={progress} setProgress={setProgress} requestTopic={"learning_MEDiml/progress/" + pageId} />}</div>
          </>
        }
      />
    </>
