...
const FlowCanvas = ({ workflowType, setWorkflowType }) => {
    ...
    (id) => {
      if (id) {
        ...
        try {
          ...
          // POST request to /extraction_MEDiml/run for the current node by sending form_data
          let formData = {
            id: id,
            name: nodeName,
          ...
          requestBackend(port, "/extraction_MEDiml/run_all/node" + pageId, formData, (response) => {
            setCanRun(true)
            if (response.error) {
              // show error message
  ...
  const onRun = useCallback(() => {
    ...
    // Post request to extraction_MEDiml/run_all for current workflow
    requestBackend(
      port, 
      "/extraction_MEDiml/run_all/" + pageId, 
      newFlow, 
      (response) => {
            ...
            <div className="panel-bottom-center">
              {isProgressUpdating && (
                <ProgressBarRequests
                  ...
                  requestTopic={"extraction_MEDiml/progress/" + pageId}
                />
              )}
            </div>
          </>
