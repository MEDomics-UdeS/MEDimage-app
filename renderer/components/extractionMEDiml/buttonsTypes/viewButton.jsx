...
const ViewButton = ({ id, data, type }) => {
  ...
  /**
   * @description
   * This function is used to send a POST request to /extraction_MEDiml/view.
   */
  const viewImage = () => {
    console.log("Viewing image for node " + id)

    // Construction of form data to send to /extraction_MEDiml/view. If the node is input, the name of the file is needed
    let formData
    ...
    setLoading(true)
    requestBackend(port, "/extraction_MEDiml/view/", formData, (response) => {
      setLoading(false)
      if (response.error) {
