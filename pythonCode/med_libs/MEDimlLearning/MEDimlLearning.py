                    ...
                    if designed_experiment:            
                        ...
                        if (content["name"].lower() == "data"):
                            ...
                            try:
                                ...
                                ml_dict_paths = MEDiml.utils.load_json(path_ml)      # Test information dictionary

                                # Machine learning information dictionary
                                ml_info_dict = dict()

                                # Training and test patients
                                ml_info_dict['patientsTrain'] = MEDiml.utils.load_json(ml_dict_paths['patientsTrain'])
                                ml_info_dict['patientsTest'] = MEDiml.utils.load_json(ml_dict_paths['patientsTest'])
            ...
            for node in pip:
                ...
                if (content["name"].lower() == "design"):
                    if not designed_experiment:
                        ...
                        f.writelines("\n# Initialize the DesignExperiment class\n")
                        f.writelines("path_settings = Path.cwd() / 'flask_server' / 'learning_MEDiml' / 'settings'\n")
                        f.writelines("experiment = MEDiml.learning.DesignExperiment(path_study, path_settings, design_settings['expName'])\n")
                ...
                if designed_experiment:            
                    ...
                    if (content["name"].lower() == "data"):                 
                        ...
                        f.writelines("\n    # Load the test dictionary and machine learning information\n")
                        f.writelines("    ml_dict_paths = MEDiml.utils.load_json(path_ml)\n")

                        f.writelines("\n    # Machine learning information dictionary\n")
                        f.writelines("    ml_info_dict = dict()\n")

                        f.writelines("\n    # Training and test patients\n")
                        f.writelines("    ml_info_dict['patientsTrain'] = MEDiml.utils.load_json(ml_dict_paths['patientsTrain'])\n")
                        f.writelines("    ml_info_dict['patientsTest'] = MEDiml.utils.load_json(ml_dict_paths['patientsTest'])\n")
                        ...
                        f.writelines("    patients_holdout = MEDiml.utils.load_json(path_study / 'patientsHoldOut.json') if holdout_test else None\n")
                    ...
                    if (content["name"].lower() == "cleaning"):
                        ...
                        f.writelines("        rad_table_learning = MEDiml.learning.ml_utils.get_radiomics_table(path_radiomics_csv, path_radiomics_txt, image_type, patient_ids)\n")
                        ...
                        f.writelines("        data_cleaner = MEDiml.learning.DataCleaner(rad_table_learning)\n")
                    ...
                    if (content["name"].lower() == "normalization"):
                        ...
                        f.writelines("            normalization = MEDiml.learning.Normalization('combat')\n")
                    ...
                    if (content["name"].lower() == "feature reduction"):
                        ...
                        f.writelines("        patients_ids = MEDiml.learning.ml_utils.intersect(patients_train, list(rad_tab.index))\n")
                        ...
                        f.writelines("\n    # Initialize the FSR class\n")
                        f.writelines("    fsr = MEDiml.learning.FSR(f_set_reduction_method)\n")
                        ...
                        f.writelines("    rad_tables_testing = MEDiml.learning.ml_utils.combine_rad_tables(rad_tables_testing)\n")
                    ...
                    if content["name"].lower() == "radiomics learner":
                        ...
                        f.writelines("\n    # Patient definitions (training and test sets)\n")
                        f.writelines("    patient_ids = list(outcome_table_binary.index)\n")
                        f.writelines("    patients_train = MEDiml.learning.ml_utils.intersect(MEDiml.learning.ml_utils.intersect(patient_ids, patients_train), rad_tables_training.index)\n")
                        f.writelines("    patients_test = MEDiml.learning.ml_utils.intersect(MEDiml.learning.ml_utils.intersect(patient_ids, patients_test), rad_tables_testing.index)\n")
                        f.writelines("    patients_holdout = MEDiml.learning.ml_utils.intersect(patient_ids, patients_holdout) if holdout_test else None\n")
                        ...
                        f.writelines("\n    # Training the model\n")
                        f.writelines("    learner = MEDiml.learning.RadiomicsLearner(path_study=path_study, path_settings=Path.cwd(), experiment_label=experiment_label)\n")
                        ...
                        f.writelines("    model_dict = MEDiml.learning.ml_utils.save_model(name_type, model, 'var1', path_model)\n")
                        ...
                        f.writelines("            path_radiomics_txt = item['txt']\n")
                        f.writelines("            image_type = item['type']\n")
                        f.writelines("            rad_table_holdout = MEDiml.learning.ml_utils.get_radiomics_table(path_radiomics_csv, path_radiomics_txt, image_type, patients_holdout)\n")
                        f.writelines("            rad_tables_holdout.append(rad_table_holdout)\n")
                            
                        f.writelines("        # Combine the tables\n")
                        f.writelines("        var_table_all_holdout = MEDiml.learning.ml_utils.combine_rad_tables(rad_tables_holdout)\n")
                        ...
                        f.writelines("\n    # E. Computing performance metrics\n")
                        f.writelines("\n    # Initialize the Results class\n")
                        f.writelines("    result = MEDiml.learning.Results(model_dict, model_id)\n")
                        f.writelines("    if holdout_test:\n")
                        ...
                        f.writelines("\n    # F. Saving the results dictionary\n")
                        f.writelines("    MEDiml.utils.json_utils.save_json(path_results, run_results, cls=NumpyEncoder)\n")
            ...
            if saved_results:
                ...
                for node in pip:
                    ...
                    if content["name"].lower() == "analyze":
                        ...
                        f.writelines("result = MEDiml.learning.Results()\n")
                        f.writelines("\n# Settings\n")
